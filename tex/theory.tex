...
\begin{figure}[hb] % Higgs couplings
 \centering
...
particles that leave no trace in collider detectors is accompanied by production of a Standard Model particle, 
which balances the transverse momentum in an event.
In this work, we consider such a scenario in which are produced a pair of charged leptons ($\ell^{+}\ell^{-}$, where $\ell=\Pe$ or $\Pgm$),
...
The $\PZ$ boson subsequently decays into two charged leptons, 
producing a low-background dilepton signature, 
together with $\met$ due to the undetected DM particles. 
In this work, the DM particle $\chi$ is assumed to be a Dirac fermion.
